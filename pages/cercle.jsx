...
export default class Cercle extends Component {
    ...
    spawnEnnemy = (n) => {
        ...
        this.setState(
            (prev) => {
                return { ennemies: [...prev.ennemies, { rotation: rotation, inside: inside }] };
            },
            () => {
    ...
    animateBall = () => {
        ...
        let id = setInterval(() => {
            if (this.state.ball.rotation >= 360)
                this.setState((prev) => {
                    return { ball: { ...prev.ball, rotation: 0 } };
                });

            this.setState((prev) => {
                return { ball: { ...prev.ball, rotation: prev.ball.rotation + 1 } };
            });
            ...
            if (
                ...
                isAfter(this.state.ball.rotation, this.state.ennemies[0].rotation) ||
                isAfter(this.state.ball.rotation + 360, this.state.ennemies[0].rotation)
            ) {
                this.setState((prev) => {
                    return { score: prev.score + 1, ennemies: prev.ennemies.slice(1) };
                });
                this.spawnEnnemy(1);
            }
    ...
    handleSwitchBall = () => {
        if (this.state.dead) {
            ...
            this.setState((prev) => {
                return { ball: { ...prev.ball, inside: !prev.ball.inside } };
            });
            this.forceUpdate();
        }
    };
                ...
                <div className="section">
                    <div className={this.state.firstGame || this.state.dead ? style.game : `${style.game} ${style.playing}`}>
                        <div
                            ...
                            style={{
                                transform: `rotate(${this.state.ball.rotation}deg) translateY(${
                                    this.state.firstGame ? 0 : this.state.ball.inside ? -9.25 : -11.25
                                }em)`,
                            }}
                        ></div>
                        {this.state.ennemies.map((ennemy) => (
                            <div
                                className={style.ennemy}
                                key={ennemy.rotation}
                                style={{ transform: `rotate(${ennemy.rotation}deg) translateY(${ennemy.inside ? -8.5 : -12}em)` }}
                            ></div>
                        ))}
                    </div>
                    ...
                    <button type="button" onKeyPress={this.handleSwitchBall} onMouseDown={this.handleSwitchBall}>
                        {this.state.firstGame ? "Commencer" : this.state.dead ? "Recommencer" : "Changer de côté"}
                    </button>
                </div>
            </div>
        );
