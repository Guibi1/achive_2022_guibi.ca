        ...
        return (
            <div className="page">
                ...
                <div className="section">
                    <div className={style.board}>
                        {this.state.plateau.map((row, rowIndex) =>
                            row.map((element, index) =>
                                <button type="button" disabled={element !== vide || this.state.gagnant !== ""} className={this.state.plateauGagnant[rowIndex][index] ? `${style.cell} ${style.win}` : style.cell} onClick={() => this.jouer(rowIndex, index)} key={index}>
                                    {element}
                                </button>
                            )
                        )}
                    ...
                    <p>{this.state.gagnant === "" ? (this.state.joueurActuel === X ? "C'est le tour aux X." : "C'est le tour aux O.") : (this.state.gagnant === vide ? "Partie nulle !" : (this.state.gagnant === X ? "Les X ont gagné !" : "Les O ont gagné !"))}</p>
