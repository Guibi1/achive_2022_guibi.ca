...
export default class Flow extends Component
{
    ...
    createGame = () =>
    {
        ...
        this.colors.slice(0, this.state.flowCount)
            .sort(() => Math.floor(Math.random() * 3) - 1)
            .forEach((color, i) => {
                let solution = []
                if (i < this.state.boardSize)
                    for (let row = 0; row < this.state.boardSize - (this.state.flowCount - this.state.boardSize); row++)
        ...
        for (let iterationCount = 0; iterationCount < flows.length * 400;)
        {
            ...
            let flow = flows.filter((f) => f.solution.length < Math.pow(this.state.flowCount, 1.2)).sort(() => Math.floor(Math.random() * 3) - 1).at(0)
            let growingIndex = Math.floor(Math.random() * 2) - 1

            // If newDot is on an existing dot
            if (flows.filter((f, i) => f != flow && f.solution.length > 3)
                .sort(() => Math.floor(Math.random() * 3) - 1)
                .some((f) => {
                    let shrinkingAt
                    if (this.pointsAreNeighboors(f.solution.at(0), flow.solution.at(growingIndex)))
                        shrinkingAt = 0
                    
                    else if (this.pointsAreNeighboors(f.solution.at(-1), flow.solution.at(growingIndex)))
                        shrinkingAt = -1
    ...
    */
    ...
    boardCompleted = () =>
    {
        if (!this.state.flows.every((f) => f.lineCompleted))
            return false
    ...
    toggleSolution = () =>
    {
        this.setState((prevState) => { return { showSolution: !prevState.showSolution } })
    }
    ...
    stopTracingFlow = () =>
    {
        if (this.state.tracingFlow == undefined)
            return
    ...
    handleMouseEnterCase = (c) =>
    {
        ...
        if ((this.pointsAreEqual(flow.start, flow.corners.at(0)) && this.pointsAreEqual(flow.end, c))
            ||
        ...
        )
        {
            ...
            else
            {
                ...
                if (flow.corners.at(-1).row != c.row && flow.corners.at(-1).column != c.column)
                    flow.corners.push(flow.lines.at(-1))
    
                this.setState(prevState => { return { movesCounter: prevState.movesCounter + 1 } })
                // Add the last point
                flow.lines.push(c)
                flow.corners.push(c)
        ...
        else if (flow.lines.some((line) => this.pointsAreEqual(line, c)))
        {
            ...
            while (!this.pointsAreEqual(flow.lines.at(-1), c))
            {
                ...
                if (this.pointsAreEqual(flow.corners.at(-1), removed))
                    flow.corners.pop()
            }
        }
            ...
            <div className="page">
                ...
                <div className="section">
                    <div className={`${style.fixedWidth} flex`}>
                        ...
                        <p>{`Complété: ${Math.floor(this.state.flows.reduce((total, current) => total + current.lines.length, 0) / (this.state.boardSize * this.state.boardSize) * 100)}%`}</p>
                    </div>
                    ...
                    <div ref={this.board} onMouseUp={this.stopTracingFlow} onMouseLeave={this.stopTracingFlow} className={style.board}>
                        {this.boardCompleted() ?
                            <div className={style.windowGameOver}>
                                <h3>Victoire !</h3>
                                <p>{`Vous avez réussi en ${Math.floor((this.state.timeEnd - this.state.timeStart) / 1000)} secondes et en ${this.state.movesCounter} coups.`}</p>
                        ...
                        {[...Array(this.state.boardSize)].map((_, row) => 
                            <div className={style.row} key={row}>
                                {[...Array(this.state.boardSize)].map((_, column) => {
                                    ...
                                    {
                                        return (
                                            <div onMouseEnter={() => this.handleMouseEnterCase(c)} onMouseDown={() => this.startTracingFlow(flow, c)} className={style.case} key={column}>
                                                <div className={style.dot} style={
                                                    {
                                                        backgroundColor: flow.color,
                                                        boxShadow: flow.lineCompleted && !this.state.showSolution ? `0 0 ${caseSize * lineThickness}px 0 ${flow.color}` : "none"
                                                    }
                                                }/>
                                            </div>
                                        )
                        ...
                        {this.state.flows.map((flow) => {
                            ...
                            const createLine = (start, end, glow) => {
                                let s = {
                                    backgroundColor: flow.color,
                                    width: start.row === end.row ? `${(Math.abs(start.column - end.column) + lineThickness) * caseSize}px` : `${caseSize * lineThickness}px`,
                                    height: start.column === end.column ? `${(Math.abs(start.row - end.row) + lineThickness) * caseSize}px` : `${caseSize * lineThickness}px`,
                                    top: start.row < end.row ? `${(start.row + (1 - lineThickness) / 2) * caseSize}px` : `${(end.row + (1 - lineThickness) / 2) * caseSize}px`,
                                    left: start.column < end.column ? `${(start.column + (1 - lineThickness) / 2) * caseSize}px` : `${(end.column + (1 - lineThickness) / 2) * caseSize}px`,
                                    borderRadius: `${caseSize * lineThickness}px`,
                                    boxShadow: glow ? `0 0 ${caseSize * lineThickness}px 0 ${flow.color}` : "none"
                                }

                                lines.push(<div style={s} className={style.line} key={`${flow.color} ${start.row};${start.column} ${end.row};${end.column}`}/>)
                            }

                            if (this.state.showSolution)
                                for (let i = 0; i < flow.solution.length - 1; i++)
                                    createLine(flow.solution.at(i), flow.solution.at(i + 1), false)

                            else
                            {
                                for (let i = 0; i < flow.corners.length - 1; i++)
                                    createLine(flow.corners.at(i), flow.corners.at(i + 1), flow.lineCompleted)
