...
class Cercle extends Component
{
    ...
    animateBall = () =>
    {
        ...
        let id = setInterval(() => {
            if (document.activeElement.id !== "cercle-game")
                return;
            ...
            <div className="section">
                ...
                <div id="cercle-game" onKeyPress={this.handleKeyPress} tabIndex={-1}>
                    ...
                    {this.state.ennemies.map((ennemy) =>
                        <div className="cercle-ennemy" key={ennemy.rotation} style={{ transform: "rotate(" + ennemy.rotation + "deg) translateY(" + (ennemy.inside ? -106 : -150) + "px)" }}></div>
                    )}
                </div>

                <div className="stack">
                    <p id="cercle-score">{this.state.firstGame ? "Appuyez sur une touche pour commencer." : `Score: ${this.state.score}`}</p>
                    <p id="cercle-click">Cliquez dans le cercle pour jouer !</p>
                </div>
            </div>
        )
    }
